...
- [x] Record in `DECISIONS.md`, with status `Accepted`, before Phase 1:
      ...
      parameters
- [x] Commit: pre-registered design (in the Phase 0 and Phase 1–2 commits)
...
- [x] Commit: "feat: price data pipeline" (combined with Phase 2)
...
- [x] `notebooks/01_pair_selection.ipynb`: visualize the candidate pairs'
      price series and spreads (formation period only)
- [x] `tests/test_cointegration.py`
- [x] Commit: "feat: cointegration testing across candidate pairs — see DECISIONS.md for full results"
...
- [x] Commit: "feat: z-score mean-reversion signal (look-ahead verified by test)"
...
- [x] Commit: "feat: gross-of-costs backtest"
...
- [x] Commit: "feat: realistic transaction cost model applied — see docs/results.md for gross vs. net comparison"
...
- [ ] (Not pursued — the README covers the full story) Write `paper/writeup.md`: a short writeup (this project doesn't
...
- [x] Write `README.md` in the exact structure required by `PROJECT.md`
...
- [x] Include the research-question quote from `PROJECT.md` §2, followed
      by the finding exactly as the pipeline produced it
- [x] Fill `DECISIONS.md` with all real decisions, including the full
      list of pairs tested (Phase 2) and cost model parameters
- [x] `CHANGELOG.md` v1.0.0 entry
- [x] Final review: does the README read like an honest research finding,
      or like a strategy pitch? If it leans toward the latter anywhere,
      rewrite that section.
- [x] Publication check: search the repo for assistant references, local
